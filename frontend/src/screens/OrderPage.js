    ...
    return (
        <>
            {order &&
                ...
                <Container maxWidth='xl' sx={{ display: 'block', minHeight: '100vh', p: 1 }}>
                    {/* HEADER */}
                    <Box display='inline-flex' sx={{ width: '100%', px: 2, alignItems: 'center', mb: 1, flexWrap: 'wrap' }}>
                        <Typography variant='h3'>ORDER # <strong>{order._id}</strong></Typography>
                        <Divider orientation='vertical' />
                        <Typography variant='body2' color='success.main' sx={{ px: 1 }}>Paid</Typography>
                        <Divider orientation='vertical' />
                    ...
                    <Box display='flex' sx={{ gap: 2, flexWrap: 'wrap' }}>
                        {/* LEFT CONTAINER */}
                        <Paper sx={{ flex: '1 2 500px', border: '1px solid #999', borderRadius: '10px', overflow: 'hidden' }}>
                            {order.orderItems && order.orderItems.map((item, index) => (
                                <Item key={index} item={item} />
                            ))}
                        </Paper>
                        {/* RIGHT CONTAINER */}
                        <Paper sx={{ flex: '0 1 350px', p: 2, border: '1px solid #999', borderRadius: '10px', overflow: 'hidden' }}>
                            ...
                            <Typography variant='body2' color='grey.500' sx={{ display: 'flex', justifyContent: 'space-between', pl: 1 }}>
                                <strong>Total</strong>
    ...
    return (
        <>
            <Box sx={{ p: 2, display: 'flex' }}>
                <Box sx={{ flex: '0 0 auto', width: '100px', aspectRatio: '16/18', bgcolor: '#FAFAFA', borderRadius: '5px' }}>
