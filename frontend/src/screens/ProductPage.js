...
const ProductPage = () => {
    ...
    const fetchData = async () => {
        ...
        // console.log(data)
        setItemName(data.name.toLowerCase().replace(/ /g, ''))


        setProduct({ ...data, itemName })
    }
    const addToCart = () => {
        setOpen(true)
    }
                        ...
                        <Container maxWidth='xl' sx={{ display: 'flex', flexWrap: 'wrap' }}>
                            ...
                            <Box display='flex' sx={{ flex: '1 3 300px', minHeight: '250px', alignItems: 'center' }}>
                                <Box display='block' sx={{ textAlign: 'center', margin: '0 auto' }}>
                                    ...
                                    <Button variant='contained' disableRipple sx={{ width: '100%' }} onClick={addToCart}>ADD TO CART</Button>
                                </Box>
                            </Box>
                        </Container>
                    </Box>
