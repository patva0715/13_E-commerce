...
const SearchFilter = () => {
    ...
    return (
        <Paper sx={{ position: 'sticky', top: '20px', py:2, px:2 }}>
            <Button variant='contained' fullWidth endIcon={open?<i className="fas fa-chevron-up"/>:<i className="fas fa-chevron-down"/>} onClick={()=>setOpen(state=>!state)} sx={{justifyContent:'space-between'}}>CATEGORY</Button>
            <Collapse in={open} timeout="auto" unmountOnExit sx={{pl:2,pt:1}}>
            <FormGroup>
